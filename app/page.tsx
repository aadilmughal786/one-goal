...
const featuresData = [
  {
    icon: <FiCommand className="w-6 h-6 text-text-primary" />,
  ...
  {
    icon: <MdOutlineRepeat className="w-6 h-6 text-text-primary" />,
...
const useCasesData = [
  {
    icon: <FaUserGraduate className="w-10 h-10 text-text-primary" />,
    persona: 'The Student',
  ...
  {
    icon: <FaCode className="w-10 h-10 text-text-primary" />,
  ...
  {
    icon: <FaPencil className="w-10 h-10 text-text-primary" />,
...
const FeatureCard = ({
  ...
  <div className="p-6 rounded-xl border backdrop-blur-sm transition-all duration-300 cursor-pointer group bg-bg-secondary border-border-primary hover:bg-bg-tertiary hover:border-border-secondary hover:scale-105">
    <div className="flex justify-center items-center mb-4 w-12 h-12 rounded-lg bg-bg-tertiary">
      {icon}
    </div>
...
const AccordionItem = ({ question, answer }: { question: string; answer: string }) => {
  ...
  return (
    <div className="py-2">
      <button
        className="flex justify-between items-center px-4 py-4 w-full text-lg font-semibold text-left rounded-lg transition-colors duration-300 cursor-pointer text-text-primary group hover:bg-bg-tertiary"
      ...
      >
        <div className="overflow-hidden">
          <div className="px-4 pt-2 pr-10 pb-4 leading-relaxed text-text-secondary">{answer}</div>
        </div>
      </div>
    </div>
  );
  ...
  const renderCtaButton = () => {
    if (authLoading) {
      return (
        <button
          disabled
          className="inline-flex gap-3 items-center px-8 py-4 font-semibold rounded-full opacity-60 cursor-not-allowed text-bg-primary bg-text-primary"
        >
          <svg
    ...
    if (currentUser) {
      return (
        <Link
          href="/dashboard"
          className="inline-flex gap-3 items-center py-2 pr-6 pl-2 font-semibold rounded-full transition-all duration-200 cursor-pointer text-bg-primary bg-text-primary group hover:opacity-90 hover:scale-105 hover:shadow-xl"
        >
    ...
    return (
      <Link
        href="/login"
        className="inline-flex gap-3 items-center px-8 py-4 font-semibold rounded-full transition-all duration-200 cursor-pointer text-bg-primary bg-text-primary group hover:opacity-90 hover:scale-105 hover:shadow-xl"
      >
        <FiTarget size={20} />
        Get Started
        <FiArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
    ...
    <>
      ...
      <div className="relative z-10 px-6 mx-auto max-w-6xl sm:px-8 lg:px-12">
        ...
        <section className="pt-20 pb-32 text-center fade-in">
          <div className="mb-8">
            <div className="inline-flex justify-center items-center mb-8 w-20 h-20 rounded-full border backdrop-blur-sm bg-bg-secondary border-border-primary">
          ...
          <p className="mx-auto mb-12 max-w-3xl text-lg leading-relaxed sm:text-xl lg:text-2xl text-text-secondary">
            Your singular focus in a world of distractions. Achieve clarity, drive action, and
            conquer your most important objectives.
          </p>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {howItWorksData.map((step, index) => (
              <div key={index} className="text-center">
                <div className="inline-flex justify-center items-center mb-6 w-16 h-16 rounded-full border backdrop-blur-sm bg-bg-secondary border-border-primary">
                  {step.icon}
                </div>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-4xl">
            ...
            <div className="p-8 transition-all duration-300 card sm:p-12 hover:border-border-secondary">
              <div className="space-y-6 text-lg leading-relaxed text-text-secondary">
                <p>
                  In today&apos;s fast-paced world, constant demands pull us in countless
                  directions. We often fall into the trap of multitasking, believing it makes us
                  more productive. However, true impact and innovation rarely come from juggling
                  many tasks at once.
                </p>
                <p>
                  <strong className="text-text-primary">One Goal</strong> is built on the principle
                  of deep, sustained focus. It&apos;s your antidote to digital noise, providing a
                  crystal-clear space to define, track, and conquer your most significant
                  aspirations.
                </p>
              </div>
            </div>
        ...
        <section className="py-20">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {useCasesData.map((useCase, index) => (
              <div
                key={index}
                className="p-8 text-center rounded-xl border backdrop-blur-sm transition-all duration-300 cursor-pointer bg-bg-secondary border-border-primary hover:bg-bg-tertiary hover:border-border-secondary"
              >
                <div className="inline-flex justify-center items-center mb-6 w-16 h-16 rounded-full bg-bg-tertiary">
                  {useCase.icon}
                </div>
                <h3 className="mb-2 text-xl font-semibold text-text-primary">{useCase.persona}</h3>
                <p className="mb-4 font-medium text-text-accent">&quot;{useCase.goal}&quot;</p>
                <p className="leading-relaxed text-text-secondary">{useCase.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-4xl">
            ...
            <div className="p-4 rounded-2xl border backdrop-blur-sm bg-bg-secondary border-border-primary sm:p-8">
            ...
            <div className="p-8 transition-all duration-300 card sm:p-12 hover:border-border-secondary">
              <div className="flex flex-col gap-8 justify-center items-center md:flex-row">
                <div className="flex-shrink-0 text-center md:text-left">
                  ...
                  <p className="mb-4 text-text-tertiary">Full Stack Developer</p>
                ...
                <div className="flex-grow space-y-4 leading-relaxed text-text-secondary">
                  <p>
                    As a developer, I&apos;ve always been drawn to creating tools that simplify
                    complexity. The idea for{' '}
                    <span className="font-medium text-text-primary">One Goal</span> emerged from a
                    personal struggle with information overload and the constant push to do more,
                    faster.
                  </p>
                  <p>
                    I realized that true productivity isn&apos;t about juggling tasks; it&apos;s
                    about the profound impact of singular, unwavering focus. This app is a testament
                    to the belief that focused effort yields extraordinary outcomes.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="py-20 text-center">
          <div className="p-8 card sm:p-12">
            <h2 className="mb-4 text-3xl font-bold text-text-primary sm:text-4xl">
              Ready to Focus?
            </h2>
            <p className="mx-auto mb-8 max-w-2xl text-lg text-text-secondary">
              Stop juggling priorities and start achieving them. Your most important goal is
              waiting.
            </p>
            <div className="flex flex-col gap-4 justify-center items-center h-16 sm:flex-row">
              {renderCtaButton()}
